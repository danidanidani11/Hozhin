...
users_payment = {}  # user_id: {"status": ..., "receipt": ..., "username": ...}
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            "خطا در بررسی عضویت کانال. لطفا بعدا امتحان کنید."
        )
        return

    await update.message.reply_text(
        f"سلام {user.first_name} 👋\n"
        "به ربات کتاب هوژین حرمان خوش آمدید. من می‌توانم به شما کمک کنم کتاب را خریداری کنید یا درباره کتاب اطلاعات کسب کنید.",
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # بخش انتقادات و پیشنهادات (هر پیام غیر از فیش به ادمین ارسال شود)
    await bot.send_message(
        chat_id=ADMIN_ID,
        ...
        if data.startswith("approve_"):
            ...
            if os.path.exists(pdf_path):
                with open(pdf_path, "rb") as f:
                    await bot.send_document(chat_id=user_id, document=InputFile(f, filename="hozhin_harman.pdf"))
                await bot.send_message(chat_id=user_id, text="پرداخت شما تایید شد. کتاب برای شما ارسال گردید. از خریدتان متشکریم! ❤️")
            else:
                await bot.send_message(chat_id=ADMIN_ID, text="فایل کتاب در سرور موجود نیست!")

        else:
            users_payment[user_id]["status"] = "rejected"
            await query.edit_message_text(f"❌ پرداخت کاربر {user_id} رد شد.")
            await bot.send_message(chat_id=user_id, text="پرداخت شما تایید نشد. لطفا دوباره فیش را ارسال کنید یا با پشتیبانی تماس بگیرید.")

dp.add_handler(CommandHandler("start", start))
dp.add_handler(CallbackQueryHandler(button_handler, pattern="^(buy|feedback|about_book|about_author|audiobook)$"))
dp.add_handler(MessageHandler(filters.ALL & (~filters.COMMAND), message_handler))
dp.add_handler(CallbackQueryHandler(admin_callback_handler, pattern="^(approve_|reject_)"))
